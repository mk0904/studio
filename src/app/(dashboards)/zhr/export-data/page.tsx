...
export default function ZHRExportDataPage() {
  const { user: zhrUser } = useAuth();
  const { toast } = useToast();

  const [isLoadingPage, setIsLoadingPage] = useState(true);
  const [isExporting, setIsExporting] = useState(false);

  const [allBhrsInZone, setAllBhrsInZone] = useState<User[]>([]);
  ...
  const fetchInitialFilterData = useCallback(async () => {
    if (!zhrUser || zhrUser.role !== 'ZHR') {
      setIsLoadingPage(false);
      setError("Access Denied or user not ZHR.");
      return;
    }
    ...
    try {
      const { data: bhrsData, error: bhrsError } = await supabase
        ...
        .eq('role', 'BHR')
        .eq('reports_to', zhrUser.id);
      if (bhrsError) throw bhrsError;
      setAllBhrsInZone(bhrsData || []);
      setBhrOptions((bhrsData || []).map(u => ({ value: u.id, label: `${u.name} (${u.e_code || 'N/A'})` })));
      ...
      console.error("Error fetching initial filter data for ZHR:", e);
  ...
  const handleExportToCSV = async () => {
    ...
    if (selectedBhrIds.length > 0) {
      ...
      targetBhrIds = allBhrsInZone.map(b => b.id); // All BHRs in ZHR's zone
    }

    if (targetBhrIds.length === 0) {
      toast({ title: "No Data", description: "No BHRs to fetch data for. Cannot export.", variant: "default" });
  ...
  return (
    <div className="space-y-8">
      <PageTitle title="Export Visit Data (ZHR)" description="Filter and export submitted visit data from your zone." />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
                ...
                <DropdownMenu>
                    ...
                    {bhrOptions.length === 0 && <DropdownMenuLabel className="text-xs text-muted-foreground px-2">No BHRs in your zone.</DropdownMenuLabel>}
                    </DropdownMenuContent>
                </DropdownMenu>
